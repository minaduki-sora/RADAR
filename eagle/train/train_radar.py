...
def make_speed_matrix_RADAR(eagen_minus_time, eaforward_time, eye_time, max_len):
    ...
    with torch.no_grad():
        for states, stop_probs, avg_acc_lens_op in tqdm(data_loader, desc="Calculating Stats", leave=False):
            ...
            for _ in range(num_samples):
                ...
                # 4. 计算本次采样的RADAR速度
                speed_RADAR = speed_matrix[acc_lens_sampled.squeeze(-1), action_length.squeeze(-1)] # [B]
                
                # 存储本次采样的结果
                batch_action_lengths.append(action_length)
                batch_eye_speeds.append(speed_RADAR)
...
def train_and_evaluate(params, train_loader, test_loader, project_root):
    ...
    # --- 奖励和速度矩阵 ---
    speed_matrix = make_speed_matrix_RADAR(params['eagen_minus_time'], params['eaforward_time'], params['eye_time'], max_len).to(device)
